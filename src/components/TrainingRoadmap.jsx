...
// Dedicated mobile path — a tall zigzag, drawn natively in a narrow
// viewBox so it never needs non-uniform stretching to fit.
const MOBILE_ROAD_D = `M 60 60 H 340 Q 380 60 380 100 V 260
  Q 380 300 340 300 H 60 Q 20 300 20 340 V 540
  Q 20 580 60 580 H 340 Q 380 580 380 620 V 820
  Q 380 860 340 860 H 60 Q 20 860 20 900 V 1100
  Q 20 1140 60 1140 H 200 V 1350`;
const MOBILE_VB_W = 400;
const MOBILE_VB_H = 1450;
...
/* [top%, left%] — desktop uses the 900x1400 viewBox */
const POS_DESKTOP = [
  [2, 6],
  [3, 58],
  [15, 50],
...
/* [top%, left%] derived directly from MOBILE_ROAD_D coordinates,
   e.g. icon0 sits at viewBox (110,60) -> (60/1450*100, 110/400*100).
   Because these come from the same viewBox as the path, the icons
   always sit exactly on the road no matter how the container stretches. */
const POS_MOBILE = [
  [2, 5],   // Enroll — shelf 1, left
  [4, 70],     // Practice Sessions — shelf 1, right
  [18, 60],    // Assignments — shelf 2, right
  [17, 5],  // Live Projects — shelf 2, left
  [35, 10],  // Practical Sessions — shelf 3, left
  [40, 75],    // Grooming Sessions — shelf 3, right
  [55, 55],    // Resume Building — shelf 4, right
  [63, 1],  // Interview Prep — shelf 4, left
  [86, 30],    // Be Job Ready — final centered stub
];
...
export default function TrainingRoadmap() {
  ...
  useEffect(() => {
    ...
    // re-measure whenever the path itself changes (desktop <-> mobile)
  }, [mobile]);

  const posFor = (i) => {
    const [top, left] = mobile ? POS_MOBILE[i] : POS_DESKTOP[i];
    return { top: `${top}%`, left: `${left}%` };
  };
  ...
  return (
    <div>
      <div
        ...
        style={mobile ? { aspectRatio: `${MOBILE_VB_W} / ${MOBILE_VB_H}` } : undefined}
      >
        <svg
          viewBox={mobile ? `0 0 ${MOBILE_VB_W} ${MOBILE_VB_H}` : "0 0 900 1400"}
          fill="none"
          preserveAspectRatio={mobile ? "none" : "xMidYMin meet"}
          className="absolute inset-0 h-full w-full overflow-visible"
        >
          <defs>
            <mask id="tp-road-mask">
              <path
                ref={maskRef}
                d={mobile ? MOBILE_ROAD_D : ROAD_D}
                stroke="#fff"
                strokeWidth={mobile ? 46 : 52}
                strokeLinecap="round"
                strokeLinejoin="round"
                vectorEffect="non-scaling-stroke"
              />
          ...
          <path
            ...
            stroke="#F8FAFC"
            strokeWidth={mobile ? 3 : 6}
            strokeDasharray={mobile ? "12 10" : "18 16"}
            strokeLinecap="round"
            mask="url(#tp-road-mask)"
            vectorEffect="non-scaling-stroke"
          />
